...
<body>
	...
	<div class="container">
		<h3 class="w3ls-title1" align="center" style="color:white;font-weight:bold;margin:15px 15px 15px 15px">
			<i class="fa fa-clock-o"></i>新增直播預告時間
		</h3>
...
<%-- 						<c:if test="${trailerExist.contains(memVO.mem_no)}"> --%>
						...
						<c:if test="${!empty trailerSvc.findByPK(memVO.mem_no)}">
						<tr>
							<td>直播預告是否開啟:</td>
							...
							<td>
								<div class="checkbox checkbox-slider--b checkbox-slider-md">
									<label>
										<input type="checkbox" value = "1" name = "trailer_status" checked >
										<input type="hidden" name="sale_no" value="${trailerSvc.findByPK(memVO.mem_no).sale_no}">
										<span>On</span>
									</label>
								</div>
							</td>
					...
					<br>
					<input type="hidden" name="action" value="${empty trailerSvc.findByPK(memVO.mem_no)? 'insert' : 'update'}">
					<div align="center">
						<input onclick="sendTrail()" class="btn btn-success btn-lg " type="submit" value="送出">
						<input class="btn btn-danger btn-lg " type="button" value="取消" onclick="location.href='<%=request.getContextPath()%><%=request.getParameter("requestURL")%>?whichPage=<%=request.getParameter("whichPage")%>'">
