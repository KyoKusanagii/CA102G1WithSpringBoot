...
<body>
	...
	<div class="container">
		<c:if test="${sdf.parse(sdf.format(limitSaleVO.getSale_end())).getTime() > sdf.parse(sdf.format(date)).getTime() and limitSaleVO.getSale_status()!=0}">
			<div id="countdown" class="ClassyCountdownDemo"></div>
		</c:if>
		<h3 class="w3ls-title1" align="center" style="color:white;font-weight:bold;margin-bottom:15px;margin-top: 15px">
			修改限時拍賣商品
		...
		<div class="login-body">
			...
			<FORM class="form-horizontal" METHOD="post" ACTION="<%=request.getContextPath() %>/FrontEnd/LimitSale.do" name="form1">
				<div class="form-group">
					<table class="table">
						...
						<tr>
							...
							<td style="font-weight:bold; font-size:18px;">
								原本開始時間: <%=sdf.format(limitSaleVO.getSale_start())%><br>
								原本結束時間: <%=sdf.format(limitSaleVO.getSale_end())%>
							</td>
						...
						<tr>
							...
							<td align="left">
								...
								<br>
								<input type="text" id="StartTime" name="limitSale_start_time" class="timepicker" style="margin-bottom: 5px;"value="<%=(limitSaleVO == null) ? "" : limitSaleVO.getSale_start()%>">
								<br>
							...
							<td>
								<div class="checkbox checkbox-slider--b checkbox-slider-md">
									<label>
										...
										<%=(limitSaleVO.getSale_status() == 1) ? "checked" : "" %>  
					...
					<div align="center">
						<input class="btn btn-success btn-lg " type="submit" value="送出修改">
						<input type="hidden" name="item_no" value="<%=limitSaleVO.getItem_no()%>">
						<input type="hidden" name="sale_no" value="<%=limitSaleVO.getSale_no()%>">
						<input class="btn btn-danger btn-lg " type="button" value="取消修改" onclick="location.href='<%=request.getContextPath()%><%=request.getParameter("requestURL")%>?whichPage=<%=request.getParameter("whichPage")%>'">
